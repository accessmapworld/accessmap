...
export const Card = forwardRef(({ customClass, ...rest }, ref) => (
  <div ref={ref} {...rest} className={`rb-card ${customClass ?? ''} ${rest.className ?? ''}`.trim()} />
))
...
const CardSwap = ({ width = 460, height = 320, cardDistance = 56, verticalDistance = 64, delay = 4500, pauseOnHover = false, skewAmount = 5, easing = 'elastic', children }) => {
  ...
  useEffect(() => {
    ...
    const swap = () => {
      ...
      rest.forEach((idx, i) => { const el = refs[idx].current; const slot = makeSlot(i, cardDistance, verticalDistance, refs.length); tl.set(el, { zIndex: slot.zIndex }, 'promote'); tl.to(el, { x: slot.x, y: slot.y, z: slot.z, duration: config.durMove, ease: config.ease }, `promote+=${i * 0.15}`) })
    ...
    swap(); intervalRef.current = window.setInterval(swap, delay)
    if (pauseOnHover) {
  ...
  return <div ref={container} className="rb-card-swap" style={{ width, height }}>{rendered}</div>
}
